...
function App() {
  ...
  const installCommandWindows = "$scriptPath = \"$env:USERPROFILE\\Documents\\PowerShell\\copilot_here.ps1\"; Invoke-WebRequest -Uri \"https://github.com/GordonBeeming/copilot_here/releases/download/cli-latest/copilot_here.ps1\" -OutFile $scriptPath; if (-not (Select-String -Path $PROFILE -Pattern \"copilot_here.ps1\" -Quiet -ErrorAction SilentlyContinue)) { Add-Content $PROFILE \"`n. $scriptPath\" }; . $PROFILE";
